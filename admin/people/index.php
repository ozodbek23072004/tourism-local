...
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM people p $whereSql");
...
$query = "
    SELECT p.id, p.name_uz, p.born_year, p.died_year, p.image, p.status, p.created_at,
            ...
            <tbody class="divide-y divide-gray-100">
                <?php if (count($people) > 0): ?>
                    ...
                    <tr class="hover:bg-gray-50/80 group">
                        <td class="px-6 py-4">
                            <?= getImageHtml($person['image'] ?? '', $person['name_uz'] ?? 'X') ?>
                        </td>
                        <td class="px-6 py-4 font-medium text-gray-800">
                            <?= htmlspecialchars($person['name_uz']) ?>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600">
                            <?= $person['born_year'] ?> – <?= $person['died_year'] ? $person['died_year'] : 'hozir' ?>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600">
                            <?= htmlspecialchars($person['region_name'] ?? '—') ?>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end space-x-2">
                                <a href="edit.php?id=<?= $person['id'] ?>" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg">Tahrirlash</a>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">Ma'lumot topilmadi.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
