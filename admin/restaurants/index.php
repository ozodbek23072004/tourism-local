...
if ($search !== '') {
    $whereClauses[] = "(r.name LIKE :search OR r.cuisine_type LIKE :search)";
    $params['search'] = "%{$search}%";
}
if ($regionId !== '') {
    $whereClauses[] = "r.region_id = :region_id";
...
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM restaurants r $whereSql");
...
$query = "
    SELECT r.id, r.name, r.price_range, r.image, r.status, r.created_at,
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    ...
                    <th class="px-6 py-4">Narx darajasi</th>
            ...
            <tbody class="divide-y divide-gray-100">
                <?php if (count($restaurants) > 0): ?>
                    ...
                    <tr class="hover:bg-gray-50/80 group">
                        <td class="px-6 py-4">
                            <?= getImageHtml($item['image'] ?? '', $item['name'] ?? 'X') ?>
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm font-bold text-green-600">
                            <?php 
                                if ($item['price_range'] === 'low') echo '$';
                                elseif ($item['price_range'] === 'mid') echo '$$';
                                elseif ($item['price_range'] === 'high') echo '$$$';
                                else echo '-';
                            ?>
                        </td>
