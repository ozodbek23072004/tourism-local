...
$stmt = $pdo->prepare("SELECT r.*, r.image_path as image, reg.name_uz as region_name, reg.name_ru as region_name_ru, reg.name_en as region_name_en FROM restaurants r LEFT JOIN regions reg ON r.region_id = reg.id WHERE r.id = :id");
$stmt->execute(['id' => $id]);
$rest = $stmt->fetch();

if (!$rest) { http_response_code(404); require_once '../404.php'; exit; }

try { $pdo->prepare("UPDATE restaurants SET views = views + 1 WHERE id = :id")->execute(['id' => $id]); } catch (PDOException $e) {}

// Gallery
$gallery = $pdo->prepare("SELECT * FROM gallery WHERE entity_type = 'restaurant' AND entity_id = :id ORDER BY id DESC");
$gallery->execute(['id' => $id]);
$galleryImages = $gallery->fetchAll();

// Related restaurants
$stmtRelated = $pdo->prepare("SELECT r.id, r.name, r.image_path as image, r.cuisine_type, r.price_level FROM restaurants r WHERE r.region_id = :region_id AND r.id != :id ORDER BY r.id DESC LIMIT 4");
$stmtRelated->execute(['region_id' => $rest['region_id'], 'id' => $rest['id']]);
$related = $stmtRelated->fetchAll();

// Nearby hotels
$nearbyHotels = [];
if ($rest['region_id']) {
    $sh = $pdo->prepare("SELECT id, name_uz, name_ru, name_en, image, stars FROM hotels WHERE region_id = :rid AND status = 'active' LIMIT 3");
    $sh->execute(['rid' => $rest['region_id']]);
    $nearbyHotels = $sh->fetchAll();
}

// Reviews stats
$revStats = $pdo->prepare("SELECT AVG(rating) as avg_r, COUNT(*) as cnt FROM reviews WHERE entity_type='restaurant' AND entity_id=:id AND status='approved'");
$revStats->execute(['id' => $id]);
$reviewStats = $revStats->fetch();

$lang = currentLang();
$restName = localizedField($rest, 'name');
$restDesc = localizedField($rest, 'description');
$regName = localizedField($rest, 'region_name');

$pageTitle = htmlspecialchars($restName);
require_once '../includes/seo.php';
$desc = mb_substr(strip_tags($restDesc), 0, 160);
$img = $rest['image'] ? BASE_URL . trim(UPLOAD_DIR, '/') . '/' . ltrim($rest['image'], '/') : null;
renderMeta(['title' => $pageTitle . ' | Silk Road Explorer', 'description' => $desc, 'image' => $img]);
require_once '../includes/layout_header.php';

$currentUrl = (isset($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];

function getPriceLabelV($range) {
    return match($range) { 
        'low' => ['$', 'Arzon', 'bg-emerald-500/10 text-emerald-600'], 
        'mid' => ['$$', "O'rtacha", 'bg-amber-500/10 text-amber-600'], 
        'high' => ['$$$', 'Qimmat', 'bg-rose-500/10 text-rose-600'], 
        default => ['', '', 'bg-silk-50 text-silk-400'] 
    };
}
[$priceIcon, $priceText, $priceClasses] = getPriceLabelV($rest['price_level'] ?? null);
?>

<!-- Hero -->
<section class="relative h-[50vh] min-h-[400px] lg:h-[60vh] bg-silk-950 overflow-hidden">
    <img src="<?= publicImage($rest['image'], $restName) ?>" alt="<?= htmlspecialchars($restName) ?>" class="w-full h-full object-cover opacity-50">
    ...
    <div class="absolute bottom-0 left-0 w-full">
        <div class="max-w-7xl mx-auto px-6 lg:px-8 pb-12">
            <div class="flex items-center gap-2 text-amber-400/60 text-sm mb-6" data-aos="fade-up">
                ...
                <a href="index.php" class="hover:text-amber-400 transition-colors"><?= __('nav_restaurants') ?></a>
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="text-amber-400"><?= htmlspecialchars(mb_substr($restName, 0, 30)) ?></span>
            </div>

            <div class="flex flex-wrap items-center gap-3 mb-5" data-aos="fade-up" data-aos-delay="100">
                <?php if(!empty($rest['cuisine_type'])): ?>
                <span class="bg-amber-500 text-silk-950 px-4 py-1.5 rounded-lg text-xs font-bold shadow-lg shadow-amber-500/25"><?= htmlspecialchars($rest['cuisine_type']) ?></span>
                <?php endif; ?>
                <?php if($priceIcon): ?>
                <span class="bg-white/10 backdrop-blur text-white px-4 py-1.5 rounded-lg text-xs font-bold border border-white/10"><?= $priceIcon ?></span>
                <?php endif; ?>
                <?php if($regName): ?>
            ...
            <h1 class="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-white" data-aos="fade-up" data-aos-delay="200"><?= htmlspecialchars($restName) ?></h1>

            <!-- Action buttons -->
            <div class="flex flex-wrap gap-3 mt-6" data-aos="fade-up" data-aos-delay="300"
                 x-data="{ isFav: false, copied: false }"
                 x-init="fetch('<?= BASE_URL ?>public/api/favorites.php?type=restaurant&id=<?= $id ?>').then(r=>r.json()).then(d=>isFav=d.is_favorite)">
                <button @click="fetch('<?= BASE_URL ?>public/api/favorites.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'restaurant',id:<?= $id ?>})}).then(r=>r.json()).then(d=>isFav=d.is_favorite)"
    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col lg:flex-row gap-12">
        ...
        <div class="flex-1 min-w-0" data-aos="fade-up">
            ...
            <article class="prose prose-lg max-w-none text-silk-700 leading-relaxed mb-12">
                <div x-show="tab==='uz'"><?= nl2br(htmlspecialchars($rest['description_uz'] ?? __('no_info'))) ?></div>
                <div x-show="tab==='ru'" style="display:none"><?= nl2br(htmlspecialchars($rest['description_ru'] ?? __('no_info'))) ?></div>
                <div x-show="tab==='en'" style="display:none"><?= nl2br(htmlspecialchars($rest['description_en'] ?? __('no_info'))) ?></div>
            </article>

            <!-- Map Section -->
            <?php if ($rest['latitude'] && $rest['longitude']): ?>
            <div class="mt-12 pt-8 border-t border-silk-200" data-aos="fade-up">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="https://www.google.com/maps/dir/?api=1&destination=<?= $rest['latitude'] ?>,<?= $rest['longitude'] ?>" target="_blank" 
                ...
                document.addEventListener('DOMContentLoaded', function(){
                    const m = L.map('detail-map',{zoomControl:false,scrollWheelZoom:false}).setView([<?= $rest['latitude'] ?>,<?= $rest['longitude'] ?>], 15);
                    ...
                    const customIcon = L.divIcon({
                        className: '',
                        html: `<div style="display:flex; align-items:center; justify-content:center; width:48px; height:48px; background:linear-gradient(135deg, #10b981, #059669); border-radius:50%; border:4px solid white; box-shadow:0 10px 25px rgba(16,185,129,0.5);">
                                 <span class="material-symbols-outlined" style="color:white; font-size:24px;">restaurant</span>
                               </div>`,
                        iconSize: [48, 48],
                        iconAnchor: [24, 48],
                        popupAnchor: [0, -50]
                    ...
                    L.marker([<?= $rest['latitude'] ?>,<?= $rest['longitude'] ?>], {icon: customIcon})
                     .addTo(m)
                     .bindPopup(`
                        <div style="text-align:center; padding:5px;">
                            <h4 style="font-weight:bold; font-size:16px; margin:0 0 5px 0; color:#2a1f18;"><?= htmlspecialchars(addslashes($restName)) ?></h4>
                            <p style="font-size:12px; color:#666; margin:0;"><?= htmlspecialchars(addslashes($rest['address'] ?? '')) ?></p>
                        </div>
                     `, {
                         closeButton: false,
            ...
            <div class="mt-12 pt-8 border-t border-silk-200" data-aos="fade-up">
                ...
                <div class="rounded-3xl overflow-hidden shadow-2xl bg-black aspect-video border border-silk-200">
                    <?php if (strpos($rest['video_url'], 'youtube.com') !== false || strpos($rest['video_url'], 'youtu.be') !== false): 
                        $ytId = '';
                        if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $rest['video_url'], $match)) {
                            $ytId = $match[1];
                        }
                    ?>
                    ...
                    <?php else: ?>
                        <video controls class="w-full h-full"><source src="<?= htmlspecialchars($rest['video_url']) ?>" type="video/mp4"></video>
                    <?php endif; ?>
                </div>
            </div>
            <?php endif; ?>
        ...
        <div class="w-full lg:w-80 shrink-0 space-y-6" data-aos="fade-left">
            ...
            <div class="bg-silk-50 rounded-2xl p-6 border border-silk-200/60">
                ...
                <ul class="space-y-4">
                    <?php if (!empty($rest['working_hours'])): ?>
                    <li class="flex items-start gap-3 text-silk-700">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-silk-400 uppercase tracking-wider mb-0.5"><?= __('work_hours') ?></p>
                            <p class="text-sm"><?= htmlspecialchars($rest['working_hours']) ?></p>
                        </div>
                    </li>
                    <?php endif; ?>
                    ...
                    <li class="flex items-start gap-3 text-silk-700">
                        ...
                        <div>
                            ...
                            <a href="tel:<?= preg_replace('/[^0-9+]/', '', $rest['phone']) ?>" class="text-sm text-amber-600 font-semibold hover:text-amber-700"><?= htmlspecialchars($rest['phone']) ?></a>
                        </div>
                    </li>
                    <?php endif; ?>
                    ...
                    <li class="flex items-start gap-3 text-silk-700">
                        ...
                        <div>
                            ...
                            <p class="text-sm"><?= htmlspecialchars($rest['address']) ?></p>
                        </div>
                    </li>
                    <?php endif; ?>
                </ul>
            ...
            <!-- Booking Form -->
            <div class="bg-gradient-to-br from-emerald-50 to-emerald-100/50 rounded-2xl p-6 border border-emerald-200/60" x-data="bookingForm()" data-aos="fade-up">
                <h3 class="font-bold text-silk-900 text-sm uppercase tracking-wider mb-5 flex items-center gap-2">
                    <span class="material-symbols-outlined text-emerald-500 text-lg">calendar_month</span>
                    <?= __('book_restaurant') ?>
                </h3>
                <div class="space-y-3">
                    <input x-model="form.guest_name" type="text" placeholder="<?= __('guest_name') ?> *" class="w-full bg-white border border-silk-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400">
                    <input x-model="form.guest_phone" type="tel" placeholder="<?= __('guest_phone') ?> *" class="w-full bg-white border border-silk-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400">
                    <div>
                        <label class="text-[10px] font-bold text-silk-400 uppercase tracking-wider mb-1 block"><?= __('check_in') ?></label>
                        <input x-model="form.check_in" type="date" :min="today" class="w-full bg-white border border-silk-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400">
                    </div>
                    <div>
                        <label class="text-[10px] font-bold text-silk-400 uppercase tracking-wider mb-1 block"><?= __('guests_count') ?></label>
                        <select x-model="form.guests_count" class="w-full bg-white border border-silk-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400">
                            <template x-for="i in 10"><option :value="i" x-text="i + ' <?= __('guests_count') ?>'"></option></template>
                        </select>
                    </div>
                    <textarea x-model="form.message" rows="2" placeholder="<?= __('booking_message') ?>" class="w-full bg-white border border-silk-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400 resize-none"></textarea>
                    <button @click="submitBooking()" :disabled="submitting" class="w-full bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-emerald-500/25 active:scale-[0.98] transition-all disabled:opacity-50 text-sm flex items-center justify-center gap-2">
                ...
                <div class="space-y-3">
                    ...
                    <a href="<?= BASE_URL ?>public/hotels/view.php?id=<?= $nh['id'] ?>" class="flex items-center gap-3 group">
                        <div class="w-12 h-12 rounded-lg overflow-hidden bg-silk-200 shrink-0">
                            <img src="<?= publicImage($nh['image'], localizedField($nh, 'name')) ?>" class="w-full h-full object-cover">
                        </div>
                        <div class="min-w-0">
                            <p class="text-sm font-semibold text-silk-800 truncate group-hover:text-amber-700 transition-colors"><?= htmlspecialchars(localizedField($nh, 'name')) ?></p>
                            <div class="flex">
                                <?php for($s=1;$s<=5;$s++): ?>
                                <span class="text-[10px] <?= $s<=(int)$nh['stars'] ? 'text-amber-400' : 'text-silk-200' ?>">★</span>
                                <?php endfor; ?>
                            </div>
                        </div>
                    </a>
...
<section class="py-16 bg-silk-50 border-t border-silk-200/60">
    <div class="max-w-7xl mx-auto px-6 lg:px-8">
        <h2 class="font-display text-2xl font-bold text-silk-900 mb-8" data-aos="fade-up"><?= __('related_rests') ?></h2>
                ...
                <a href="view.php?id=<?= $rel['id'] ?>" class="bg-white rounded-2xl overflow-hidden border border-silk-200/60 card-hover group" data-aos="fade-up" data-aos-delay="<?= $i * 100 ?>">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center justify-between mt-2">
                            <p class="text-[10px] text-silk-400 font-medium"><?= htmlspecialchars($rel['cuisine_type'] ?? '') ?></p>
                            <?php 
                            [$pIcon, $pText, $pClasses] = getPriceLabelV($rel['price_level'] ?? null);
                            if ($pIcon): 
                            ?>
                            <span class="text-[10px] font-bold text-amber-600"><?= $pIcon ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                </a>
...
function reviewForm() {
    return {
        ...
        async loadReviews() {
            const r = await fetch('<?= BASE_URL ?>public/api/reviews.php?type=restaurant&id=<?= $id ?>');
        ...
        async submitReview() {
            ...
            await fetch('<?= BASE_URL ?>public/api/reviews.php', {
                ...
                body: JSON.stringify({type:'restaurant', id:<?= $id ?>, name:this.newName, rating:this.newRating, comment:this.newComment})
            });
...
function bookingForm() {
    return {
        ...
        form: { guest_name:'', guest_phone:'', check_in:'', guests_count:1, message:'' },
        ...
        async submitBooking() {
            ...
            try {
                const r = await fetch('<?= BASE_URL ?>public/api/bookings.php', {
                    method:'POST', headers:{'Content-Type':'application/json'},
                    body: JSON.stringify({...this.form, entity_type:'restaurant', entity_id:<?= $id ?>})
                });
                const d = await r.json();
                if (d.success) {
                    this.successMsg = '<?= __('booking_success') ?>';
                    this.form = {guest_name:'',guest_phone:'',check_in:'',guests_count:1,message:''};
                } else {
